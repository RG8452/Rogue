...
public class MainMenu extends JPanel
{
	...
	public MainMenu() // Constructor which just puts a mouseListener on the panel and makes it focusable
	{
		...
		addButton(new Button(screenX - 200, 100, 100, 100, "Options", Color.gray, Color.white, 7) {
			...
			{
				int tX = this.getX(); // Retrieve useful variables
				int tY = this.getY();
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			for (Button butt : buttons) // Loop through every button
			{
				if (butt.inBounds(e.getX(), e.getY())) // If that button was clicked
				{
					...
					{
						case "Play":
							// This next stuff has to do with thread pooling.
							// Essentially, hitting "Play" calls up a new thread which begins to run through actual gameplay.
							try
							{

...
public class PausePanel extends JPanel
{
	...
	public PausePanel()
	{
		...
		addButton(new Button(MainMenu.screenX - 200, 100, 100, 100, "Options", Color.gray, Color.white, 7) {
			...
			{
				int tX = this.getX();
				int tY = this.getY(); // Retrieve useful variables
	...
	protected void paintComponent(Graphics g) // Overridden repainting for the Pausecomponent
	{
		...
		g2d.fillRect(0, 0, 5000, 3000); // Fills in a rect with high alpha to get fade
	...
	private class PauseHandler implements KeyListener, MouseListener
	{
		// keyListener methods
		...
		public void keyTyped(KeyEvent e) // Basic printing method
		{
			...
			{
				Startup.getGUI().swapPanels(new DevPanel((PausePanel) Startup.getGUI().getPanel()));
			}
		}
